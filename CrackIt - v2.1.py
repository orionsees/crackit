    ...
    if os.path.exists(pathDIR):
        ...
        def GetFiles():
            ...
            for qNo in range(int(len(final_QNo))):
                ...
                resize_list.append(_basewidth_[int(final_QNo[qNo])])
        ...
        def CheckCOL():
            ...
            if endTEST[-1] == 1:
                ...
                for i in range(length):
                    if i != int(lockedBTN[-1]):
                        if int(EntertimeQ[i]) == 1:
                            omrBtnList[i].configure(image=omrBtn_answered, fg="white")
                            omrBtnList[i].image = omrBtn_answered
                        elif int(EntertimeQ[i]) == 0:
                            omrBtnList[i].configure(image=visited_unanwered, fg="white")
                            omrBtnList[i].image = visited_unanwered
                        else:
                            omrBtnList[i].configure(image=omrBtn_unanswer, fg="white")
                            omrBtnList[i].image = omrBtn_unanswer
                    else:
                        omrBtnList[i].configure(image=omrBtn_selected, fg="#6fa5fd")
                        omrBtnList[i].image = omrBtn_selected
                PutCorrectOpt()
...
def StartTest(event):
    ...
    entry = ttk.Entry(inputWin1, justify=CENTER, font=("Bahnschrift Light SemiCondensed", 40), validate="key", validatecommand=(validation, '%S'))
    entry.focus_force()
    entry.pack(side=TOP, fill=X, padx=5, pady=5)
    enterBTN = Label(inputWin1, text='START TEST', bg='cyan', fg='gray20', font=("Bahnschrift Light SemiCondensed", 18))
